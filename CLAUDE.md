...
GrandForge is a browser-based chess analysis platform: React 18 + Vite 5 + TypeScript frontend, Stockfish (16/17/18) WASM running in a Web Worker, and one Express API (`api/_lib/router.ts`) **dual-deployed** — persistent server on Render (primary, `server/index.ts`) + Vercel serverless function (fallback, `api/[...path].ts`) — backed by MongoDB Atlas.

## Commands

```bash
npm run dev          # concurrently runs the API (tsx server/index.ts, :3000) and Vite web (:5173)
...
Route modules live under `api/_lib/routes/**`, each exporting a default Express app built by `createApp()` (CORS allowlist + 5 MB JSON + rate limit 150/15min — deliberately **per route module** (~25 buckets/IP): one game review legitimately fires ~160 positions requests, which would blow a single global 150 bucket; shared-store limiting is Phase 6). They are **all mounted behind one regex dispatch table in `api/_lib/router.ts`**, which has **two deployment entry points**: (1) the *single* Vercel Serverless Function `api/[...path].ts` (Vercel Hobby allows at most **12 functions**, so ~25 one-file-per-function routes would fail to deploy; `api/_lib/**` is underscore-prefixed so Vercel never functionizes those modules — and `api/` must stay at repo root for this convention), and (2) `server/index.ts`, the persistent server Render runs (`render.yaml` blueprint; also `npm run dev` locally — listens on `PORT ?? API_PORT ?? 3000`, graceful SIGTERM drain, 120 s keepAliveTimeout for Render's proxy). Dev, Render, and Vercel all run one identical routing table — **to add a route, drop it under `api/_lib/routes/**` and register one line in `api/_lib/router.ts`** (each inner app registers its full `/api/...` path).

- **Dual-deploy client contract**: `.env.production` (committed, public values only) sets `VITE_API_BASE_URL` to the Render base; `src/services/apiBase.ts` resolves primary/fallback, probes `{primary}/health` at boot (4 s timeout — doubles as the Render warm-up ping), and `apiClient.ts` interceptors fail over stickily to same-origin `/api` on network/502/503/504 errors with a 5-min recovery re-probe. Unset `VITE_API_BASE_URL` (dev, Playwright) short-circuits all of it to `/api`. Renaming the Render service must update `render.yaml` + `.env.production` + the vercel.json CSP together (see `docs/deploy-render.md`).
- CORS: `api/_lib/corsOrigins.ts` builds the allowlist — `FRONTEND_URL`, CSV `CORS_EXTRA_ORIGINS`, plus localhost 5173/4173 always (unit-tested; vitest include covers `api/_lib/**/*.test.ts`).
- DB connection: `api/_lib/db.ts` — `connectDB()` caches the mongoose connection and shares one in-flight connect promise (concurrent handlers at game load would otherwise race a half-open socket since `bufferCommands: false`). It loads `.env` manually (`loadLocalEnv`); `dbName` is always `chess-analyzer`. Pool is environment-keyed on `process.env.VERCEL`: serverless 5/10 s, persistent 20/45 s.
