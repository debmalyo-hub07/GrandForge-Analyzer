...
- **D3 — Repo layout: light restructure only.** New `server/` folder holds the persistent entry + deploy config. `api/_lib/**` stays where it is: Vercel's functions convention requires root-level `api/`, and moving it would break the free serverless fallback. A full frontend/backend split is deliberately deferred; revisit only if we leave Vercel.
